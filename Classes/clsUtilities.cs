...
internal class ClsUtilities
{
    ...
    {
        var clsType = typeof(FrmTimeCalc);
    ...
    { /* Testdaten:
        */
        ...
        if (DateTime.TryParseExact(timeString, formats, new CultureInfo("de-DE"), DateTimeStyles.None, out var time)) { return time; }
        int hour = 0, minute = 0;
        var pattern = @"^(?:" +
    ...
    { /* Testdaten:
       ...
       */
        ...
        if (DateTime.TryParseExact(dateString, formats, new CultureInfo("de-DE"), DateTimeStyles.None, out var date)) { return date; }
    ...
    {
        for (var row = 0; row < gridView.RowCount; row++)
        {
            var isEmpty = true;
            for (var col = 0; col < gridView.ColumnCount; col++)
            {
