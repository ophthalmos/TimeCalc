...
public partial class FrmTimeCalc : Form
{
    ...
    private const int VK_MULTIPLY = 0x6A; // const int VK_OEM_2 = 0xBF; // #
    ...
    private readonly ArrayList arrColumnLefts = []; // Used to save left coordinates of columns
    private readonly ArrayList arrColumnWidths = []; // Used to save column widths
    ...
    private readonly string defaultCellStyleFontName;
    private readonly int rowHeadersWidth;
    ...
    private readonly List<int> ci_0123DateTimeAny = [0, 1, 2, 3];  // Index der Felder, in denen Datumsangaben möglich sind (z.B. mit "F5")
    ...
    private void DGV_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
    {
        ...
        {// nur wenn sich die Anzahl der Stellen ändert
            numRows = e.RowIndex;
            var Sz = e.Graphics.MeasureString(rowText, e.InheritedRowStyle.Font, 0, new StringFormat(StringFormatFlags.MeasureTrailingSpaces));
    ...
    protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
    {
        switch (keyData)
        {
            ...
            case Keys.Apps:
                ...
                {// EditMode ist ausgeschlossen => ActiveControl == null!
                    var ccell = dGV.CurrentCell;
                    var r = ccell.DataGridView.GetCellDisplayRectangle(ccell.ColumnIndex, ccell.RowIndex, false);
    ...
    private void ChangeCellValueByKey(bool shiftMode, bool addMode)
    {
        ...
        {
            var tbText = ((TextBox)dGV.EditingControl).Text;
            var dtFormat = ci_123TimeAny.Contains(dGV.CurrentCell.ColumnIndex) ? timeFormat : dateFormat;
        ...
        try
        {
            var cVal = dGV.CurrentCell.Value;
            {
                var dtFormat = ci_123TimeAny.Contains(dGV.CurrentCell.ColumnIndex) ? timeFormat : dateFormat; // letzteres für "Datum"
    ...
    {// Width = 444 //MessageBox.Show(dGV.Columns[2].Width.ToString()); // => primär 105
        ...
        var args = Environment.GetCommandLineArgs(); // The first element in the array contains the file name of the executing program. If the file name is not available, the first element is equal to string.Empty. 
        if (args.Length > 1 && args[1].Length > 0 && File.Exists(args[1])) // wenn args.Length == 2 dann existieren args[0] und args[1]
        {
            //isFileLoading = true;
            if (ReadTextFile(args[1])) { Text = currFile + " - " + winTitle; }  // Text = Path.GetFileName(currFile) + " - " + winTitle;
    ...
    {// Event ist nicht in den letzten beiden Spalten auslösbar, da die diese READONLY sind. => CellLeave-Ereignis
        ...
        {
            for (var i = 1; i < 3; i++) { dGV.Rows[e.RowIndex].Cells[i].Value = null; }
        }


        var oldString = dGV.Tag == null ? "" : dGV.Tag.ToString();
        var newString = dGV.Rows[e.RowIndex].Cells[e.ColumnIndex].Value == null ? "" : dGV.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString();
    ...
    {// aktualisiert u.a. Spalte 'Saldo'
        var ccell0 = dGV.Rows[myCell.RowIndex].Cells[0];
        var ccell1 = dGV.Rows[myCell.RowIndex].Cells[1];
        var ccell2 = dGV.Rows[myCell.RowIndex].Cells[2];
        var ccell3 = dGV.Rows[myCell.RowIndex].Cells[3];

        if (myCell.ColumnIndex == ci_1Von && ccell1.Value != null)
        {// 1. Zelle normalisieren
            var date1 = ClsUtilities.NormalizeTime(ccell1.Value.ToString());
        ...
        {// 2. Zelle normalisieren
            var date2 = ClsUtilities.NormalizeTime(ccell2.Value.ToString());
        ...
        {// 3. Zelle normalisieren
            var date3 = ClsUtilities.NormalizePause(ccell3.Value.ToString());
        ...
        {// 0. Zelle normalisieren
            var date0 = ClsUtilities.NormalizeDate(ccell0.Value.ToString());
    ...
    {// Spalte 'Saldo' und 'Spanne' werden komplett aktualisiert!
        ...
        for (var row = 0; row < dGV.RowCount - 1; ++row)
        {
            var dgvCell1 = dGV.Rows[row].Cells[1]; // von
            var dgvCell2 = dGV.Rows[row].Cells[2]; // bis
            var dgvCell3 = dGV.Rows[row].Cells[3]; // Pause
            var dgvCell4 = dGV.Rows[row].Cells[4]; // Faktor
            var dgvCell5 = dGV.Rows[row].Cells[5]; // Spanne

            if (dgvCell1.Value != null && dgvCell2.Value != null && dgvCell4.Value != null && rgxValidTime.Match(dgvCell1.Value.ToString()).Success
                && rgxValidTime.Match(dgvCell2.Value.ToString()).Success && int.TryParse(dgvCell4.Value.ToString(), out var rowFactor))
            {// DateTime d1 = DateTime.MinValue; DateTime d2 = DateTime.MinValue; DateTime d3 = DateTime.MinValue;
                ...
                {// prüfen ob beide Zellen verwertbar sind
                    ...
                    var myMinute = 0; // 24:00 ist keine gültige Zeit! Deshalb wird ParseExact umgangen!
    ...
    {// ContextMenu bei RightClick anzeigen
        var eRow = e.RowIndex; var eCol = e.ColumnIndex;
        ...
        {// MessageBox.Show(eRow.ToString() + " | " + eCol.ToString());
            ...
            var clickedCell = dGV.Rows[eRow].Cells[eCol];
    ...
    private void LinkLblDecText_Click(object sender, EventArgs e)
    {
        ...
        {
            var cbText = linkLblDecText.Text.Replace(decText, "");
            try
            {
                if (cbText.Length > 0)
                {
    ...
    {// MessageBox.Show(dGV.SelectionMode.ToString());
        ...
        else
        {
            ...
            for (var r = 0; r < dGV.RowCount; r++) { dGV[e.ColumnIndex, r].Selected = true; }
        }
    }
    ...
    {// NICHT wenn Cursor in letzter (neu angelegter) Zeile steht!
        ...
        {
            if (dGV.IsCurrentCellInEditMode) { dGV.EndEdit(); }
            //var oldRowIndex = dGV.CurrentCellAddress.X;
    ...
    {// dGV.Focus(); darf hier nicht stehen weil Zelle in EditMode sonst den Fokus verliert!
        if (!dGV.IsCurrentCellInEditMode)
        {
            var iCol = dGV.CurrentCell.ColumnIndex;
            var iRow = dGV.CurrentCell.RowIndex;
    ...
    private void MainMenuStrip_MenuActivate(object sender, EventArgs e)
    {
        ...
        if (dGV.CurrentCell.ColumnIndex != 2) // dGV.CurrentCell.EditType == typeof(DataGridViewComboBoxEditingControl funkt nicht in EditMode
        {
            ...
            var dtFormat = ci_12VonBis.Contains(dGV.CurrentCell.ColumnIndex) ? timeFormat : dateFormat; //string[] dtFormats = { "H:mm", "d.M.yyyy" };
    ...
    private void ÖffnenToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        if (openFileDialog.ShowDialog() == DialogResult.OK)
        {
            ...
            if (ReadTextFile(openFileDialog.FileName)) { Text = currFile + " - " + winTitle; }
    ...
    private void FrmTimeCalc_FormClosing(object sender, FormClosingEventArgs e)
    {
        if (!nothingToSave && e.CloseReason == CloseReason.UserClosing) // && !clsUtilities.isDGVEmpty(dGV))
        {
            if (currFile.Length > 0)
            {
                var dlgResult = MessageBox.Show("Möchten Sie die Änderungen an " + currFile + " speichern?", winTitle, MessageBoxButtons.YesNoCancel, MessageBoxIcon.Exclamation);
                switch (dlgResult)
                {
    ...
    private DataTable CreateDataTable()
    {
        ...
        for (var i = 0; i < dGV.ColumnCount - 2; ++i)
        {
        ...
        for (var row = 0; row < dGV.RowCount - 1; row++)
        {
            ...
            for (var col = 0; col < dGV.Columns.Count - 2; col++) { dr[col] = dGV.Rows[row].Cells[col].Value != null ? dGV.Rows[row].Cells[col].Value.ToString() : ""; }
            dtData.Rows.Add(dr);
        }
        return dtData;
    }
    ...
    private bool SaveTextFile(string fullFilename)
    {
        ...
        var dtData = CreateDataTable();
        using (DataSet ds = new()
        {
            DataSetName = "TimeCalc"
        }) //Finally the save part:
        {
            ds.Tables.Add(dtData);
            var dtCopy = dtDays.Copy();
            ds.Tables.Add(dtCopy); // ds.Tables.Add(dtDays); => DataTable already belongs to another DataSet
    ...
    private void Control_KeyDown(object sender, KeyEventArgs e)
    {
        ...
        {
            numPadDecimal = true;
            var iPos = tB.SelectionStart; // Cursorpositon in TextBox
    ...
    {// This event occurs after the KeyDown event and can be used to prevent characters from entering the control
        ...
        else if (dGV.CurrentCell.EditType == typeof(DataGridViewTextBoxEditingControl))
        {
            ...
            {
                DateTime dt; // = new DateTime();
                var dtFormat = ci_12VonBis.Contains(dGV.CurrentCell.ColumnIndex) || dGV.CurrentCell.ColumnIndex == ci_3Pause ? timeFormat : dateFormat;
                var strTxtBox = txtBox.TextLength == 0 ? DateTime.Now.ToString(dtFormat) : txtBox.Text;
    ...
    private void WebseiteToolStripMenuItem_Click(object sender, EventArgs e)
    {
        try
        {
            ProcessStartInfo psi = new("https://www.netradio.info/timecalc/") { UseShellExecute = true };
            Process.Start(psi);
        }
...
";
    ...
    private void ToolStripButtonComplete_Click(object sender, EventArgs e)
    {
        ...
        {// if (!dGV.IsCurrentCellInEditMode) { dGV.BeginEdit(true); }
            ...
            DateTime dt; // = new DateTime();
            var dtFormat = ci_12VonBis.Contains(dGV.CurrentCell.ColumnIndex) ? timeFormat : dateFormat;
            var strTxtBox = txtBox.TextLength == 0 ? DateTime.Now.ToString(dtFormat) : txtBox.Text;
            if (ci_12VonBis.Contains(dGV.CurrentCell.ColumnIndex)) { dt = ClsUtilities.NormalizeTime(strTxtBox); }
            else { dt = ClsUtilities.NormalizeDate(strTxtBox); }
            if (dt != DateTime.MinValue)
    ...
    private void AddShortcutToDesktop()
    {
        ...
        {
            var deskText = currFile.Length > 0 ? Path.GetFileName(currFile) : ClsUtilities.GetDescription();
            var linkFileName = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.Desktop), deskText + ".lnk");
    ...
    private void ToolStripMenuItemPrint_Click(object sender, EventArgs e) => ShowPrintDialog();
    ...
    public void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
    {
        ...
        try
        {
            ...
            var bMorePagesToPrint = false; // Whether more pages have to print or not
            var iTmpWidth = 0;
            if (bFirstPage)
            ...
            {// Loop till all the grid rows not get printed
                ...
                else
                {
                    ...
                    {// Draw Header
                        ...
                        var strDate = DateTime.Now.ToShortDateString() + ", " + DateTime.Now.ToShortTimeString();
    ...
    private void ToolStripMenuItemExcel_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            if (currFile.Length > 0)
            {
                var filePath = Path.Combine(Path.GetDirectoryName(currFile), Path.GetFileNameWithoutExtension(currFile) + "_Excel.csv");
                ...
                {
                    var dr = dt.NewRow();
                    for (var col = 0; col < dGV.Columns.Count; col++) { dr[col] = dGV.Rows[row].Cells[col].Value != null ? dGV.Rows[row].Cells[col].Value.ToString() : ""; }
                    dt.Rows.Add(dr);
                }
                //var count = 50; // Datei muss mind. 7 KB groß sein, sonst meldet Microsoft Excel Fehler
                //if (dt.Rows.Count < count)
                //{
                //    count -= dt.Rows.Count;
                //    for (var i = 0; i < count; i++) { dt.Rows.Add("", "", "", "", "", "", ""); }
                //}
        ...
        try
        {
            ...
            if (Clipboard.ContainsText())
            {
                ...
                if (text.Length > 1)
                {
                    ...
                    var textArray = text.Split('\r'); //  create an array of lines
                    oldFormat = textArray[0].Contains(";Notiz;"); // stellt Kompatiblität zu alter Version her
                    foreach (var sLine in textArray)
                    {
        ...
        try
        {
            ...
            using (StreamReader sReader = new(fullFilename))
            {
                ...
                while ((sLine = sReader.ReadLine()) != null)
                {
                    ...
                    {
                        var splitted = sLine.Split(';', StringSplitOptions.TrimEntries); //MessageBox.Show(string.Join(Environment.NewLine, splitted));
                    ...
                    else if (!string.IsNullOrEmpty(sLine) && intVon >= 0 && intBis >= 0 && intPause >= 0)
                    {
                        ...
                        if (!string.IsNullOrEmpty(varVon) && !string.IsNullOrEmpty(varBis))
                        {
                            ...
                            var varDatum = string.IsNullOrEmpty(varDate1) ? "" : (varDate1 == varDate2 ? varDate1 : string.Concat(varDate1.AsSpan(0, 2), "/", varDate2));
        ...
        try
        {
            ...
            using (StreamReader sReader = new(fullFilename))
            {
                ...
                while ((sLine = sReader.ReadLine()) != null)
                {
                    ...
                    {
                        var splitted = sLine.Split(separators, StringSplitOptions.TrimEntries); //MessageBox.Show(string.Join(Environment.NewLine, splitted));
            ...
            if (Clipboard.ContainsText())
            {
                ...
                if (text.Length > 1)
                {
                    ...
                    foreach (var sLine in textArray)
                    {
                        if (!string.IsNullOrEmpty(sLine) && sLine[..sLine.IndexOf(';')] is "Montag" or "Dienstag" or "Mittwoch" or "Donnerstag" or "Freitag" or "Samstag" or "Sonntag")
                        {
                            var splitted = sLine.Split(separators, StringSplitOptions.TrimEntries);
                            if (!string.IsNullOrEmpty(splitted[1])) // Datum
                            {
                                match = rgxImportDate.Match(splitted[1]); // Regex.Match(sLine, @"(?<=([^;]*;){2,2})[\d:]+(?=\s)").Value
                                if (match.Success && DateTime.TryParse(match.ToString(), out result))
    ...
    {
        var scale = Size.Width / (float)MinimumSize.Width;
    ...
    private void AnalyzeMonthData()
    {
        ...
        else
        {
            ...
            for (var row = 0; row < dGV.RowCount - 1; row++)
            {
                if (DateTime.TryParse(dGV.Rows[row].Cells[0].Value.ToString(), out var myDate) && TimeSpan.TryParse(dGV.Rows[row].Cells[5].Value.ToString(), out var myTime))
                {
                    var dr = dt.NewRow();
                    dr[0] = myDate;
            ...
            {
                totalMinutes += v.Sum;
                var summary = TimeSpan.FromMinutes(v.Sum);
                var average = TimeSpan.FromMinutes(v.Average);
            ...
            var totalMinAverage = TimeSpan.FromMinutes(totalMinutes / monthQuery.Count());
    ...
    private void AnalyzeWeekData()
    {
        ...
        else
        {
            ...
            for (var i = 1; i < dGV.RowCount - 1; ++i)
            {
            ...
            {
                var year = Convert.ToDateTime(v.Date).Year;
                var foo = ISOWeek.ToDateTime(year, v.Key, DayOfWeek.Monday);
                var bar = ISOWeek.ToDateTime(year, v.Key, DayOfWeek.Sunday);
            ...
            var totalMinAverage = TimeSpan.FromMinutes(totalMinutes / weekQuery.Count());
            using QuerySummary ReportForm = new();
    ...
    {
        dGV.Sort(new RowComparer(SortOrder.Ascending));
    }

    private void DescendingToolStripMenuItem_Click(object sender, EventArgs e)
    {
        dGV.Sort(new RowComparer(SortOrder.Descending));
    }
    ...
    private void FindDuplicatesToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        for (var i = 0; i < dGV.RowCount - 1; i++)
        {
            ...
            string[] abc = [current.Cells[0].Value.ToString(), current.Cells[1].Value.ToString(), current.Cells[2].Value.ToString(), current.Cells[3].Value.ToString(), current.Cells[4].Value.ToString()];
            for (var j = i + 1; j < dGV.RowCount; j++)
            {
                var compare = dGV.Rows[j];
                ...
                {
                    string[] def = [compare.Cells[0].Value.ToString(), compare.Cells[1].Value.ToString(), compare.Cells[2].Value.ToString(), compare.Cells[3].Value.ToString(), compare.Cells[4].Value.ToString()];
            ...
            try
            {
                ...
                for (var j = i + 1; j < dGV.RowCount; j++)
                {
                    ...
                    if (current.Cells[0].Value != null && current.Cells[1].Value != null && current.Cells[2].Value != null &&
                        ...
                        DateTime.TryParse(current.Cells[1].Value.ToString(), out var currentStart) &&
                        DateTime.TryParse(current.Cells[2].Value.ToString(), out var currentStopp) &&
                        DateTime.TryParse(compare.Cells[1].Value.ToString(), out var compareStart) &&
                        DateTime.TryParse(compare.Cells[2].Value.ToString(), out var compareStopp))
                    {
                        var currentPause = DateTime.ParseExact(current.Cells[3].Value.ToString(), timeFormat, deCulture).TimeOfDay; // vorhandene Pause
                        var comparePause = DateTime.ParseExact(compare.Cells[3].Value.ToString(), timeFormat, deCulture).TimeOfDay; // vorhandene Pause
                        var completePause = currentPause + comparePause;
    ...
    private void BreaksAutoFillToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        for (var i = 0; i < dGV.RowCount - 1; i++)
        {
            if (dGV.Rows[i].Cells[3].Value.ToString() == "00:00" && dGV.Rows[i].Cells[5].Value != null && TimeSpan.TryParse(dGV.Rows[i].Cells[5].Value.ToString(), out var _9Time) && _9Time > TimeSpan.FromHours(9)) { _9Indices.Add(i); }
            else if (dGV.Rows[i].Cells[3].Value.ToString() == "00:00" && dGV.Rows[i].Cells[5].Value != null && TimeSpan.TryParse(dGV.Rows[i].Cells[5].Value.ToString(), out var _6Time) && _6Time > TimeSpan.FromHours(6)) { _6Indices.Add(i); }
        }
        ...
        {
            foreach (var j in _9Indices) { dGV.Rows[j].Cells[3].Value = "00:45"; }
            foreach (var j in _6Indices) { dGV.Rows[j].Cells[3].Value = "00:30"; }
            UpdateLastTimeColumns();
        }
        else if (_9Indices.Count == 0 & _6Indices.Count == 0) { MessageBox.Show("Bei einer Arbeitszeit > 6 Stunden ist eine 30-Minuten-Pause verpflichtend. Bei einer Arbeitszeit > 9 Stunden: 45 Minuten.\n\nDiese Tabelle enhält bereits alle erforderliche Pausen!", winTitle, MessageBoxButtons.OKCancel, MessageBoxIcon.Information); }

    }

    private void DGV_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
    {
        dGV.FirstDisplayedScrollingRowIndex = dGV.Rows[^1].Index;
    }
    ...
    private void ShowStatisticsMenuItem_Click(object sender, EventArgs e)
    {
        AnalyzeMonthData();
    }
    ...
    private void AbsencesToolStripMenuItem_Click(object sender, EventArgs e)
    {
        ...
        {
            for (var i = 0; i < dtDays.Rows.Count; i++)
            {
                AbsencesForm.Dgv.Rows[i].SetValues([dtDays.Rows[i][0], dtDays.Rows[i][1], dtDays.Rows[i][2]]);
            }
        }
        if (AbsencesForm.ShowDialog() == DialogResult.OK)
        {
            dtDays = new DataTable("Days");
            for (var i = 0; i < AbsencesForm.Dgv.ColumnCount; ++i)
            {
            ...
            for (var row = 0; row < AbsencesForm.Dgv.RowCount; row++)
            {
                var dr = dtDays.NewRow();
                for (var col = 0; col < AbsencesForm.Dgv.Columns.Count; col++) { dr[col] = AbsencesForm.Dgv.Rows[row].Cells[col].Value != null ? AbsencesForm.Dgv.Rows[row].Cells[col].Value.ToString() : ""; }
                dtDays.Rows.Add(dr);
            }
            ShowCellChangeInTitle();
        }
    ...
    private void IntroductionMenuItem_Click(object sender, EventArgs e)
    {
        try
        {
            var readmePdf = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "TimeCalc.pdf");
